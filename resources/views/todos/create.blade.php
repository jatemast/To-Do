...
@section('content')
    ...
    <div class="max-w-lg mx-auto mt-8 p-6 bg-white shadow-lg rounded-lg">
        <h1 class="text-2xl font-bold mb-4 text-center text-gray-800">Create a New To-Do</h1>
        
        <form action="{{ route('todos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf

            <div class="flex flex-col">
                <label for="title" class="text-sm font-semibold mb-1 text-gray-600">Title:</label>
                <input type="text" name="title" id="title" class="border border-gray-300 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400" required>
            </div>

            <div class="flex flex-col">
                <label for="description" class="text-sm font-semibold mb-1 text-gray-600">Description:</label>
                <textarea name="description" id="description" rows="3" class="border border-gray-300 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400" required></textarea>
            </div>

            <div class="flex flex-col">
                <label for="due_date" class="text-sm font-semibold mb-1 text-gray-600">Due Date:</label>
                <input type="date" name="due_date" id="due_date" class="border border-gray-300 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>

            <div class="flex flex-col">
                <label for="tags" class="text-sm font-semibold mb-1 text-gray-600">Tags:</label>
                <input type="text" name="tags" id="tags" class="border border-gray-300 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>
